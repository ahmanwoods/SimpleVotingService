...
public class QuestionService {
    ...
    public QuestionEntity addQuestion(String questionString) throws ResourceInUseException {
        ...
        // Disregard if the question already exists either via matching question content or UUID.
        if (questionRepository.existsByQuestion(questionString) || UUID_REGEX.matcher(questionString).matches())
    ...
    public List<QuestionEntity> getUserUnanswered(String userId) throws EntityNotFoundException {
        ...
        // Retrieve the full list of question IDs and the list of IDs for questions the user has voted on.
        // Then we return the full list, subtracting the user's votes to yield the unanswered list.
    ...
    public QuestionEntity updateQuestion(String questionString, String newQuestion) throws EntityNotFoundException, ResourceInUseException {
        ...
        // Don't allow updating a question to a UUID
        if (UUID_REGEX.matcher(newQuestion).matches())
            throw new ResourceInUseException();
